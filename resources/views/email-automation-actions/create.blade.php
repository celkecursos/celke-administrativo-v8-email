...
@section('content')
    ...
    <x-breadcrumb title="Cadastrar Ação Automatizada" :items="[
        ['label' => 'Dashboard', 'url' => route('dashboard.index')],
        ['label' => 'Ações Automatizadas', 'url' => route('email-automation-actions.index')],
        ['label' => 'Cadastrar Ação Automatizada'],
    ]" />
    ...
    <div class="content-box">
        ...
        <form action="{{ route('email-automation-actions.store') }}" method="POST">
            @csrf

            <div class="mb-4">
                <label for="name" class="form-label">Nome *</label>
                <input type="text" name="name" id="name" class="form-input"
                    placeholder="Digite o nome da ação" value="{{ old('name') }}" required>
                @error('name')
