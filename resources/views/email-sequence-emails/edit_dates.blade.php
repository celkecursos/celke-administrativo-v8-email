                        ...
                        <div class="sidebar-card-header">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Defina o tempo de espera antes de enviar este e-mail</p>
                                </div>
                            </div>
                        </div>

                        <div class="p-4">
                            <form action="{{ route('email-sequence-emails.update-dates', ['emailMachine' => $emailMachine->id, 'sequence' => $sequence->id, 'email' => $email->id]) }}" method="POST">
                                @csrf
                                @method('PUT')

                                <input type="hidden" name="email_machine_sequence_id" value="{{ $sequence->id }}">
                                <!-- Preservar valores de data fixa -->
                                <input type="hidden" name="use_fixed_send_datetime" value="{{ $email->use_fixed_send_datetime ?? 0 }}">
                                <input type="hidden" name="fixed_send_datetime" value="{{ $email->fixed_send_datetime ? $email->fixed_send_datetime->format('Y-m-d H:i:s') : '' }}">
                                <!-- Preservar valores de janela de envio -->
                                <input type="hidden" name="send_window_start_hour" value="{{ $email->send_window_start_hour }}">
                                <input type="hidden" name="send_window_start_minute" value="{{ $email->send_window_start_minute }}">
                                <input type="hidden" name="send_window_end_hour" value="{{ $email->send_window_end_hour }}">
                                <input type="hidden" name="send_window_end_minute" value="{{ $email->send_window_end_minute }}">

                                <div class="grid grid-cols-3 gap-4 mb-4">
                                    <div>
                                        <label for="delay_day" class="form-label">Dias</label>
                                        <input type="number" name="delay_day" id="delay_day" class="form-input"
                                            placeholder="0" value="{{ old('delay_day', $email->delay_day) }}" min="0">
                                    </div>
                                    <div>
                                        <label for="delay_hour" class="form-label">Horas</label>
                                        <input type="number" name="delay_hour" id="delay_hour" class="form-input"
                                            placeholder="0" value="{{ old('delay_hour', $email->delay_hour) }}" min="0" max="23">
                                    </div>
                                    <div>
                                        ...
                                        <input type="number" name="delay_minute" id="delay_minute" class="form-input"
                                            placeholder="0" value="{{ old('delay_minute', $email->delay_minute) }}" min="0" max="59">
                                    </div>
                                </div>
                        ...
                        <div class="sidebar-card-header">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Agendar envio para data e hora específica</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <form action="{{ route('email-sequence-emails.update-dates', ['emailMachine' => $emailMachine->id, 'sequence' => $sequence->id, 'email' => $email->id]) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="use_fixed_send_datetime" value="1" 
                                            {{ old('use_fixed_send_datetime', $email->use_fixed_send_datetime) ? 'checked' : '' }}
                                            class="form-checkbox">
                                        <span class="ml-2 text-gray-700 dark:text-gray-300">Usar data fixa de envio</span>
                                    </label>
                                </div>

                                <div class="mb-4">
                                    <label for="fixed_send_datetime" class="form-label">Data e Hora</label>
                                    <input type="datetime-local" name="fixed_send_datetime" id="fixed_send_datetime" class="form-input"
                        ...
                        <div class="sidebar-card-header">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Defina o horário permitido para envio do e-mail</p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <input type="number" name="send_window_start_hour" class="form-input"
                                                placeholder="Hora (0-23)" value="{{ old('send_window_start_hour', $email->send_window_start_hour) }}" 
                                                min="0" max="23">
                                        </div>
                                        <div>
                                            <input type="number" name="send_window_start_minute" class="form-input"
                                                placeholder="Minuto (0-59)" value="{{ old('send_window_start_minute', $email->send_window_start_minute) }}" 
                                                min="0" max="59">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <input type="number" name="send_window_end_hour" class="form-input"
                                                placeholder="Hora (0-23)" value="{{ old('send_window_end_hour', $email->send_window_end_hour) }}" 
                                                min="0" max="23">
                                        </div>
                                        <div>
                                            <input type="number" name="send_window_end_minute" class="form-input"
                                                placeholder="Minuto (0-59)" value="{{ old('send_window_end_minute', $email->send_window_end_minute) }}" 
                                                min="0" max="59">
                                        </div>
                                    </div>
                                </div>
